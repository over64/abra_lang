...
object Ast0 {
  ...
  case class Prop(from: Expression, prop: lId) extends Expression {
    override val info: AstInfo = from.info
  }
  ...
  case class While(info: AstInfo, cond: Expression, _then: Block) extends Expression with BlockExpression

  case class Store(info: AstInfo, to: Seq[lId], what: Expression) extends BlockExpression
