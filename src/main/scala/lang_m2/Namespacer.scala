  ...
  def mixNamespaces(module: Module, imports: Seq[Namespace]): Namespace = {
    ...
    module.typeDecls.foreach { td =>
      def typeHintToType(th: TypeHint): Type = th match {
        ...
        case FnTypeHint(args, ret) => FnPointer(args.map { arg =>
      ...
      case (sth, fnMap) =>
        ...
        fnMap.foreach {
          case (fnName, fnCont) => fnCont.fnInfo match {
            case InferedFn(_package, th, lowFn) => byPkg.put(fnName, FnContainer(HeaderFn(_package, th, Ast1.HeaderFn(lowFn.name, lowFn._type.fnPointer))))
            case _ =>
          }
        }
    }
