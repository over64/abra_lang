...
class _07And_Test extends FunSuite with LowUtil {
  val testBase = "/tmp/"
  ...
  test("And: left only") {
    ...
    mod.assertRunEquals(exit = Some(0), stdout = Some("world"))
  }
  ...
  test("And: left and right") {
    ...
    mod.assertRunEquals(exit = Some(0), stdout = Some("helloworld"))
  }
}
