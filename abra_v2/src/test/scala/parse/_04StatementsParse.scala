...
class _04StatementsParse extends FunSuite {
  ...
  test("store") {
    withStr("x: Int = 1", Store(ScalarTh(Seq(), "Int", Seq.empty), Seq(lId("x")), lInt("1")))
    withStr("x = 1", Store(AnyTh, Seq(lId("x")), lInt("1")))
    withStr("x.y.z = 1", Store(AnyTh, Seq(lId("x"), lId("y"), lId("z")), lInt("1")))
    withStr("m(0, 0) = 1", SelfCall("set", Prop(lId("m"), Seq.empty), Seq(lInt("0"), lInt("0"), lInt("1"))))
  }
